...
static void flush_large_watches(kissat *solver,
...
static void resume_watching_binaries_after_elimination(kissat *solver,
      litwatches *binaries) {
  assert(binaries);
#ifdef LOGGING
...
static void completely_resume_watching_binaries(kissat *solver,
      litwatches *binaries) {
  assert(binaries);
#ifdef LOGGING
  size_t resumed_watching = 0;
